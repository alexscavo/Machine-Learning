...
def logpdf_GMM(X, gmm):
    ...
    for w, mu, C in gmm:  # con gmm calcolo i parametri
        logpdf_conditional = logpdf_GAU_ND(X, mu, C)     # calcolo la conditional probability
...
if __name__ == '__main__':
    ...
    X = numpy.load('GMM_data_4D.npy')
    gmm = load_gmm('GMM_4D_3G_init.json')
    llPrecomputed = numpy.load('GMM_4D_3G_init_ll.npy')
    ll = logpdf_GMM(X, gmm)
    print (numpy.abs(ll-llPrecomputed).max()) # Check max absolute difference
    
    X = numpy.load('GMM_data_1D.npy')
    gmm = load_gmm('GMM_1D_3G_init.json')
    llPrecomputed = numpy.load('GMM_1D_3G_init_ll.npy')
    ll = logpdf_GMM(X, gmm)
    print (numpy.abs(ll-llPrecomputed).max()) # Check max absolute difference
    ...
    print ('LBG + EM - final average ll - pretrained model: %.8e' % (logpdf_GMM(X, load_gmm('GMM_4D_4G_EM_LBG.json')).mean()))
    print(gmm) # you can print the gmms
    print(load_gmm('GMM_4D_4G_EM_LBG.json')) # you can print the gmms
    print ('Max absolute ll difference w.r.t. pre-trained model over all training samples:', (numpy.abs(logpdf_GMM(X, gmm) - logpdf_GMM(X, load_gmm('GMM_4D_4G_EM_LBG.json')))).max())
    ...
    gmm = train_GMM_LBG_EM(X, 4)
    print ('LBG + EM - final average ll: %.8e (%d components)' % (logpdf_GMM(X, gmm).mean(), len(gmm)))
    print ('LBG + EM - final average ll - pretrained model: %.8e' % (logpdf_GMM(X, load_gmm('GMM_1D_4G_EM_LBG.json')).mean()))
    print(gmm) # you can print the gmms
    print(load_gmm('GMM_1D_4G_EM_LBG.json')) # you can print the gmms
    print ('Max absolute ll difference w.r.t. pre-trained model over all training samples:', (numpy.abs(logpdf_GMM(X, gmm) - logpdf_GMM(X, load_gmm('GMM_1D_4G_EM_LBG.json')))).max())
